...
export function CitySelector() {
  ...
  const selectedLabel = selectedCity === 'all' ? 'All Cities' : selectedDc ? selectedDc.name : CITY_SCOPE_CONFIG[selectedCity]?.label || selectedCity
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-56">
        ...
        <DropdownMenuItem onSelect={() => setSelectedCity('all')} className="gap-3 py-2.5">
          <span
            className={cn('flex h-7 w-7 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600')}
          >
        ...
        {dispatchCenters
          .filter(dc => dc.id !== 'other')
          .map(dc => (
            <DropdownMenuItem key={dc.id} onSelect={() => setSelectedCity(dc.id)} className="gap-3 py-2.5">
              <span
                className={cn(
